...
## What Works (as of 2025-04-02 - Input Language Setting Added)
...
- **Settings UI:**
  ...
  - UI elements for API key, hotkey, microphone selection, **input language selection**, and cost estimation display exist visually.
...
- **Core Logic (Partial):**
  - Settings (API Key, Hotkey, Microphone, **Language**, Usage Duration) are saved and loaded persistently using `electron-store` via `SettingsStore.js`.
  ...
  - Transcription service (`TranscriptionService.js`) communicates with OpenAI API and uses the **input language setting**.
...
- **Testing:** Thorough testing of the new input language feature across different languages and the "Auto-Detect" setting. Verify accuracy improvements where applicable. Test edge cases (e.g., invalid language code saved somehow).
...
- **Core Functionality Implemented:** The main workflow (hotkey -> record -> transcribe (with language option) -> paste) is functional. Settings are persistent. UI elements are connected. ES Module conversion complete.
- **Refinement Needed:** Focus shifts to testing, improving user experience (like transcription window closure), enhancing error handling, and preparing for distribution.
- **Documentation Updated:** Memory Bank reflects the current state including the new language feature.
